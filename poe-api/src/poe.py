...
def request_with_retries(method, *args, **kwargs):
    logger.info(f"Post: {args, kwargs}")
...
class Client:
    ...
    def setup_session(self):
        ...
        if self.client_identifier:
            self.session = requests_tls.Session(
        ...
        for i in range(attempts):
            ...
            if query_name == "recv":
                r = request_with_retries(
                    self.session.post, self.gql_recv_url, data=payload, headers=headers
                )
                return None
            else:
                r = request_with_retries(
            ...
            if data["data"] == None:
                logger.warn(
                    f'{query_name} returned an error: {data["errors"][0]["message"]} | Retrying ({i+1}/{attempts})'
                )
                time.sleep(2)
                continue
    ...
    def send_message(
        self, chatbot, message, with_chat_break=False, timeout=20, recv=False
    ):
    ...
    def delete_message(self, message_ids):
        ...
        result = self.query("DeleteMessageMutation", {"messageIds": message_ids})
